...
(function () {
  ...
  /* ---------------------------------------------------------------------
     1. Theme (light / dark) with persistence + system default
     --------------------------------------------------------------------- */
  ...
  /* ---------------------------------------------------------------------
     2. Mobile navigation — accessible disclosure
     --------------------------------------------------------------------- */
  ...
  if (filterBar) {
    ...
    function setCount(n) {
      if (countEl) countEl.textContent = n + (n === 1 ? " study" : " studies");
    }
  ...
  /* ---------------------------------------------------------------------
     6. Contact form — client-side validation only (no backend on Pages)
     --------------------------------------------------------------------- */
  ...
  if (form) {
    form.addEventListener("submit", function (e) {
      ...
      var body = [
        "Name: " + decodeURIComponent(get("name")),
        "Email: " + decodeURIComponent(get("email")),
        "",
        decodeURIComponent(get("message"))
      ].join("\n");
  ...
  /* ---------------------------------------------------------------------
     8. Hero canvas — plasmonic near-field motif
        A lattice of metal nanoparticles with an incident wave; particles
        light up as the wavefront passes, evoking near-field coupling.
        Purely decorative, aria-hidden, disabled for reduced motion.
     --------------------------------------------------------------------- */
  ...
  if (canvas && canvas.getContext) {
    ...
    function build() {
      ...
      for (var r = 0; r < rows; r++) {
        for (var c = 0; c < cols; c++) {
          ...
          var jitterY = (Math.sin(c * 39.3468 + r * 11.135) * 24634.6345) % 1;
          particles.push({
            x: c * spacing + jitterX * 16 - 8,
            y: r * spacing + jitterY * 16 - 8,
            r: 1.1 + Math.abs(jitterX) * 1.9,
            ph: Math.abs(jitterY) * Math.PI * 2
          });
        }
      }
    }
    ...
    function draw() {
      ...
      // travelling wavefronts (incident field)
      var k = 0.0135;
      for (var w = 0; w < 3; w++) {
      ...
      // nanoparticle lattice with near-field excitation
      for (var i = 0; i < particles.length; i++) {
        var p = particles[i];
        var excite = 0.5 + 0.5 * Math.sin(p.x * k - t * 2.1 + p.ph);
        var glow = Math.pow(excite, 3);
        var rad = p.r * (1 + glow * 0.85);
    ...
    // pause when off-screen or tab hidden (battery friendly)
